...
def backup(*args):
    ...
    # check collection arguments
    all_collection_names = [x['name'] for x in config.snoop_collections]
    if not options.backup_collections:
        assert not options.collections, "can't set both --no-collections and --collection C"
    for name in options.collections:
        assert name in all_collection_names, f'collection "{name}" does not exist'
    ...
    for collection_name in (options.collections or all_collection_names):
